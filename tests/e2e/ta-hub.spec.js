...
test.describe('Instructor TA Hub UI', () => {
    ...
    test('applying a role preset updates all six checkboxes in one action', async ({ page }) => {
        ...
        await expect(page.locator('.notification.success')).toContainText('Applied "Content TA" role', { timeout: 15_000 });
        ...
        await expect(page.locator('.notification.success').last()).toContainText('Flagged Content access enabled', { timeout: 15_000 });
        await expect(card.locator('.role-preset-picker select')).toHaveValue('custom');
    });
});
